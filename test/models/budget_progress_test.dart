...
void main() {
  ...
  group('BudgetProgress.fraction', () {
    test('normal spend returns correct fraction', () {
      const p = BudgetProgress(budget: budget500, spent: 250.0);
      expect(p.fraction, closeTo(0.5, 0.001));
    });

    test('over-budget fraction is clamped to 1.0', () {
      const p = BudgetProgress(budget: budget500, spent: 600.0);
    ...
    test('zero limit returns 0.0 without throwing', () {
      ...
      const p = BudgetProgress(budget: zeroBudget, spent: 10.0);
  ...
  group('BudgetProgress.isOverBudget', () {
    test('returns false when under limit', () {
      const p = BudgetProgress(budget: budget500, spent: 400.0);
      expect(p.isOverBudget, isFalse);
    });

    test('returns true when at or over limit', () {
      const p = BudgetProgress(budget: budget500, spent: 500.0);
  ...
  group('BudgetProgress equality', () {
    test('two instances with same fields are equal', () {
      const a = BudgetProgress(budget: budget500, spent: 100.0);
      const b = BudgetProgress(budget: budget500, spent: 100.0);
      expect(a == b, isTrue);
